...
export const fetchResponse = async (chats: Message[]) => {
  try {
    ...
    if (error instanceof TypeError && error.message.includes("fetch")) {
      ...
      throw new Error(
        "An unexpected error occurred. Please try again or use your own API key."
      );
    }
  }
};
...
const fetchWithServerApi = async (message: string) => {
  ...
  if (!response.ok) {
    if (response.status === 401) {
      throw new Error(
        "Server API key is invalid or expired. Please try using your own API key."
      );
    } else if (response.status === 429) {
      throw new Error(
      ...
      throw new Error(
        "Server endpoint not found. Please try again later or use your own API key."
      );
    } else {
      throw new Error(
